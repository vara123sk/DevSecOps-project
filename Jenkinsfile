pipeline {
    ...
    stages {
        ...
        stage('Docker  Build') {
            steps {
                sh 'docker build -t varask123/sprint-boot-app:v1.$BUILD_ID .'
                sh 'docker image tag varask123/sprint-boot-app:v1.$BUILD_ID varask123/sprint-boot-app:latest'
            }
        }
        stage('Image Scan') {
            steps {
                sh ' trivy image --format template --template "@/usr/local/share/trivy/templates/html.tpl" -o report.html varask123/sprint-boot-app:latest '
            }
        }
        ...
        stage('Docker  Push') {
            steps {
                withVault(configuration: [skipSslVerification: true, timeout: 60, vaultCredentialId: 'vault-cred', vaultUrl: 'http://your-vault-server-ip:8200'], vaultSecrets: [[path: 'secrets/creds/docker', secretValues: [[vaultKey: 'username'], [vaultKey: 'password']]]]) {
                    ...
                    sh 'docker push varask123/sprint-boot-app:v1.$BUILD_ID'
                    sh 'docker push varask123/sprint-boot-app:latest'
                    sh 'docker rmi varask123/sprint-boot-app:v1.$BUILD_ID varask123/sprint-boot-app:latest'
                }
            }
        }
        stage('Deploy to k8s') {
